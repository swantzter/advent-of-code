...
for (let ivx = 0; ivx < 100; ivx++) {
  for (let ivy = 0; ivy < 100; ivy++) {
    let x = 0, y = 0
    let vx = ivx, vy = ivy
    ...
    while (x < target.xmax && y > target.ymax) {
      ...
      if (
        ...
        trajectories.push({ vx: ivx, vy: ivy, ymax })
        break
      }
    }
  }
...
trajectories.sort((a, b) => b.ymax - a.ymax)

console.log(trajectories[0].ymax)
